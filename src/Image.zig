...
pub fn rgbaF32InitFromReader(
    ...
    return .{
        ...
        .uncompressed_byte_length = data_len * @sizeOf(f32),
...
// XXX: just always store as bytes but cast when needed?
pub fn dataAsBytes(self: Image) []u8 {
    if (self.dataIsF32s()) {
        return std.mem.sliceAsBytes(self.data.f32s);
    } else {
        return self.data.u8s;
    }
}
...
pub fn assertIsUncompressedRgbaF32(self: Image) void {
    if (self.encoding != .rgba_f32) {
        std.debug.panic("expected {} found {}", .{ Encoding.rgba_f32, self.encoding });
    }
...
pub fn rgbaF32Premultiply(self: Image) void {
    const zone = Zone.begin(.{ .src = @src() });
    defer zone.end();
...
pub fn rgbaF32Resized(self: Image, options: ResizeOptions) ResizeError!Image {
    ...
    return .{
        ...
        .uncompressed_byte_length = data.len * @sizeOf(f32),
        .data = .{ .f32s = data },
        .hdr = self.hdr,
        .encoding = .rgba_f32,
        .supercompression = .none,
...
pub fn rgbaF32PreserveAlphaCoverage(self: Image, options: PreserveAlphaCoverageOptions) void {
    const zone = Zone.begin(.{ .src = @src() });
    defer zone.end();
    self.assertIsUncompressedRgbaF32();
    ...
    var curr_threshold: f32 = options.threshold;
    {
        ...
        for (0..options.max_steps) |_| {
            ...
            const coverage = self.rgbaF32AlphaCoverage(options.threshold, curr_scale);
...
fn rgbaF32EncodeRgbaU8Ex(self: *@This(), gpa: Allocator, srgb: bool) EncodeRgbaU8Error!void {
    ...
    self.* = .{
        ...
        .uncompressed_byte_length = data.len,
        .data = .{ .u8s = data },
        .allocator = gpa,
        .supercompression = .none,
    };
...
fn encodeBc7Ex(
...

    {
        const encode_zone = Zone.begin(.{ .name = "encode", .src = @src() });
        defer encode_zone.end();
        if (!bc7_encoder.encode(&params, self.width, self.height, self.data.f32s.ptr)) {
            return error.EncoderFailed;
        }
    }

    self.allocator.free(self.data.f32s);
    const data = bc7_encoder.getBlocks();
    self.* = .{
        .width = self.width,
        .height = self.height,
        .uncompressed_byte_length = data.len,
...
pub fn compressZlib(
    ...
    var compressed = b: {
        const alloc_zone = Zone.begin(.{ .name = "alloc", .src = @src() });
        defer alloc_zone.end();
        break :b try std.ArrayListUnmanaged(u8).initCapacity(gpa, self.dataAsBytes().len);
    };
    ...
    _ = try compressor.write(self.dataAsBytes());
